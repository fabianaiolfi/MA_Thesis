...
worms_explainer <- "Curves apply smoothing (“loess”) to make the curve easier to read. Graphs are sorted by the average year-on-year percent increase. The region with the highest average year-on-year increase is displayed in the top left, with further regions displayed from left to right in decreasing order of average increase. The region with the lowest increase, i.e., highest decrease, is displayed on the last row."
```

\pagebreak

```{r, child = 'data_analysis/01_introduction.Rmd'}
```
...
```{r, child = 'data_analysis/05_discussion.Rmd'}
```

\pagebreak

```{r, child = 'data_analysis/06_sources.Rmd'}
```
