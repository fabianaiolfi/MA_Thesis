...
- Table \@ref(tab:results-emigration-incumbent-vote-share) displays the results of modelling average change of number of people per institution seperately on incumbent support
- Model 0 only includes the emigration rate as an independent variable and is added in Table \@ref(tab:results-emigration-incumbent-vote-share) as a reference model
- The service cuts by themselves show mixed results. 
- Service cuts to schools (school buildings and the classroom proxy) display negative coefficients
- The change in number of hospitals has no effect, and the change in number of third places shows a positive effect. This indicates that less third places is associated with an increase in incumbent vote share
...
- The results from models 5 and 6 do not differ much to models 1 through 4. Again, only school service cuts show negative associations, and no results are statistically significant

- TO DO: discuss control variable coefficient results
- TO DO: Expain the change in emigration effect but also that is stay relatively large and significant!
- TO DO: discuss $R2$ values!
...
```{r results-emigration-incumbent-vote-share, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Emigration interacted with Service Cuts on Incumbent Vote Share"}
...
model_table <- modelsummary(list("Model 0" = fe_lm_0,
             ...
             coef_map = c(emigration_election_year_per_1000 = "Emigration since last election",
                          `average_ratio_schools_election_year` = "Avg. Children per School",
...
# Interactions just using Service Cuts (no control variables)
# Using average ratio between elections
fe_lm_0 <- feols(vote_change ~
                   emigration_election_year_per_1000 |
                   nuts2016 + year,
...
```{r results-emigration-incumbent-vote-share-interaction, eval=FALSE, fig.cap="Emigration…", message=FALSE, warning=FALSE, include=FALSE}
model_table <- modelsummary(list("Model 0" = fe_lm_0,
                                 "Model 1" = fe_lm_1,
                                 "Model 2" = fe_lm_2,
                                 "Model 3" = fe_lm_3,
             ...
             title = "DV: Change in Incumbent Vote Share",
             coef_map = c(emigration_election_year_per_1000 = "Emigration since last election",
                          `emigration_election_year_per_1000:average_ratio_schools_election_year` = "Emigration since last election × Avg. Children per School",
...
```{r results-emigration-incumbent-vote-share-control-interaction, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Emigration interacted with Service Cuts on Incumbent Vote Share with Control Variables"}
model_table <- modelsummary(list("Model 0" = fe_lm_0,
                                 "Model 1" = fe_lm_1,
                                 "Model 2" = fe_lm_2,
                                 "Model 3" = fe_lm_3,
