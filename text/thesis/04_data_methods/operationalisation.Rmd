...
df <- ro_schools %>% 
  ...
  mutate(ratio_schools = round(ratio_schools, 2)) %>% 
  mutate(ratio_schools_diff = round(ratio_schools_diff, 2)) %>% 
  mutate(average_ratio_schools_election_year = round(average_ratio_schools_election_year, 2)) %>% 
  arrange(year) %>% 
  mutate(average_ratio_schools_election_year = as.character(average_ratio_schools_election_year)) %>% 
  mutate(average_ratio_schools_election_year = case_when(is.na(average_ratio_schools_election_year) == T ~ "",
  ...
  dplyr::rename(Year = year,
                ...
                `Average Change in Children per School between Elections` = average_ratio_schools_election_year)

kable(df,
      caption = "Service Cut Operationalisation Example in Bihor County, Romania (RO111)",
      format = "latex", booktabs = TRUE, escape = FALSE) %>% 
  kable_styling(font_size = 9) %>% 
  column_spec(column = 1, width = "1cm") %>% 
  column_spec(column = c(2,4), width = "1.8cm") %>% 
  column_spec(column = c(3,5,6), width = "3cm") %>% 
  add_footnote("Romanian national parliamentary elections took place in 2004 and in 2008.", notation = "none")
...
df <- ro %>% 
  ...
  dplyr::rename(Year = year,
                ...
                `Average Emigration Rate between Elections` = emigration_election_year_per_1000)

kable(df,
      caption = "Emigration Rate Operationalisation Example in Bihor County, Romania (RO111)",
      format = "latex", booktabs = TRUE, escape = FALSE) %>% 
  kable_styling(font_size = 9) %>% 
  column_spec(column = 1, width = "1cm") %>% 
  column_spec(column = c(2,3,4), width = "1.8cm") %>% 
