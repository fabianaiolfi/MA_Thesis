...
Emigration Rate
- In operationalising emigration rate, I base my approch on @dancygierEmigrationRadicalRight2022
- I only take the number of departures from a NUTS region into account, in order to isolate the effect of emigration from other demographic factors that may influence the association between population decline and incumbent support [@dancygierEmigrationRadicalRight2022 5]
...
- I operationalise emigration rate as the sum of emigrants in a region between two elections [@dancygierEmigrationRadicalRight2022 17], divided by the average population in that region between the elections. I then multiply this value by 1000 in order to get the value per 1000 inhabitants. 
...
- i operationalise electoral volatility by calculating the pedersen index [@pedersenElectoralVolatilityWestern1979] for each nuts region for every election
...
- Table \@ref(tab:ro-pedersen-example) shows electoral volatility in an examplary romanian NUTS region
...
kable(ro_volatility_2012,
  ...
  add_footnote("RO111 has a Pedersen Index of 73.5 in the year 2012. Full party name can be accessed through partyfacts.herokuapp.com/data/partycodes/Party ID/", notation="alphabet")
```
