...
As the dependent variable I use the incumbent vote change between national parliamentary elections (see p. \pageref{passage:incumbent-vote-share-change} for its operationalisation). The model uses the total emigration rate from a region between two elections as the independent variable (see p. \pageref{passage:emigration-rate} for its operationalisation). I model Poland and Romania seperately, using a fixed effects OLS estimation. Results for both models are displayed in Table \@ref(tab:emigration-incumbent-vote-share).
...
Based on these preliminary results, I proceed by building models that focus on Romania, as these data have all pointed in the right direction, supporting my assumptions. I discard Poland from this point onwards, because emigration shows no effect on anti-incumbent voting.
