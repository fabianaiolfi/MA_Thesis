...
As with the previous preliminary analysis, I again create a two-way fixed effect linear regression model, using NUTS regions and years as fixed effects. For the model’s dependent variable I use the incumbent vote change between national parliamentary elections (see p. \pageref{passage:incumbent-vote-share-change} for its operationalisation). The model uses the total emigration rate from a region between two elections as the independent variable (see p. \pageref{passage:emigration-rate} for its operationalisation). I model Poland and Romania seperately. Results for both models are displayed in Table \@ref(tab:emigration-incumbent-vote-share).

Emigration does not have a significant effect on incumbent voting in Poland. Also, the coefficient is positive, which indicates that an increase in emigration is associated with an increase in incumbent vote share. The Romanian model displays a result that aligns with the theoretical expectations. In Romania, higher emigration rates between elections are associated with a relatively strong decrease in incumbent vote share. An increase of one emigration per thousand inhabitants of a NUTS3 region is associated with a decrease of incumbent vote share of about $6.5\%$. The coefficient displays statistical significance at conventional levels. Furthermore, the Romanian model displays relatively high $R^2$ values, indicating that the model explains a large part of the dependent’s variable overall variance.
